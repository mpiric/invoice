	...
	public function dokitcheninward()
	{
		...
		if(isset($_POST["product_id_arr"]) && $_POST["product_id_arr"]!='')
		{
			...
			foreach ($product_id_arr as $store_product_id) {
				...
				$data = array();
				$data['store_product_inward_id'] = $store_product_id;
				...
				if(isset($_POST['inward_qty_'.$store_product_id]) && $_POST['inward_qty_'.$store_product_id]!="" && $_POST['inward_qty_'.$store_product_id]!=NULL)
				{
					//echo 'if not null';
					$data['inward_qty'] = $_POST['inward_qty_'.$store_product_id];	
						
					//echo 'spii:'.$data['store_product_inward_id'];
					...
					if(isset($_POST['remaining_qty_'.$store_product_id]) && $_POST['remaining_qty_'.$store_product_id]!="")
					{
						...
						$instock_data = array();
						$instock_data['store_product_inward_id'] = $data['store_product_inward_id'];
						$instock_data['next_day_instock'] = $data['remaining_qty'];
						//$instock_data['inward_qty'] = $data['inward_qty'];
					...
					if(isset($_POST['waste_qty_'.$store_product_id]) && $_POST['waste_qty_'.$store_product_id]!="")
					{
						...
						if($insert_id>0)
						{
							...
							$filterdate = date("Y-m-d",strtotime($data['created'])); 
								...
								$this->storeinward_model->update_store_instock_kitchen($data,$filterdate);
							//}
