		...
		if($is_validate == TRUE)
		{
			...
			if($result > 0)
			{
				...
				if(!empty($branchDetails))
				{
					...
					$store_products = $this->storeproduct_model->get_data();
					//print_r($store_products);

					if(!empty($store_products))
					{
						foreach ($store_products as $storeProducts) 
						{
							$storeData = array();
							$storeData['branch_id'] = $result;
							$storeData['store_product_id'] = $storeProducts['store_product_id'];
							$storeData['price'] = $storeProducts['price'];

							$this->storeproduct_model->insert_data_store_product_inward($storeData);
						}
					}
