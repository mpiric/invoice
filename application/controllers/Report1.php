...
class Report extends CI_Controller
{
	...
	public function branchwise_brand()
	{
		if(isset($_POST['branch_id']) && $_POST['branch_id']!= '')
		{
			...
			$response = array();
			$response['status'] = "1";
			$response['brand_list_by_branch'] = $this->brand_model->brand_list_by_branch($_POST['branch_id']);
    ...
    $pdf->SetSubject('Item Wise Sales Report');
     ...
     $pdf->SetHeaderData('', '', 'Item Wise Sales Report', '', array(0,64,255), array(0,64,128));
...
// EOD;
  ...
  $file_name = 'item_wise_sales_report.pdf';
   ...
   // $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE.' 001', PDF_HEADER_STRING, array(0,64,255), array(0,64,128));
     $pdf->SetHeaderData('', '', 'Waiter Commission Report', '', array(0,64,255), array(0,64,128));
    $pdf->setFooterData(array(0,64,0), array(0,64,128)); 
...
// EOD;
    ...
    public function report_sales_pdf()
    {
		...
		else
		{
			...
			$reportHtml .= ' <div class="ng-binding"><table id="sales_table" class="table table-condensed table-hover"><thead><tr><th class="hidden-xs">Created</th><th class="hidden-xs">Bill No.</th><th class="hidden-xs">Net Amt.</th><th class="hidden-xs">Discount</th> ';
			...
			foreach ($result as $order_data) 
			{
				...
				$reportHtml .= '<tr ng-repeat="row in saleslist" class="ng-scope"><td class="hidden-xs ng-binding">'.$order_data['created'].'</td><td class="hidden-xs ng-binding">'.$order_data['order_code'].'</td><td class="hideen-xs ng-binding">'.$order_data['sub_total'].'</td><td class="hideen-xs ng-binding">'.$order_data['discount'].'</td>';
			...
			$reportHtml .= '<tr><td><b>Total:</b></td><td></td><td><b class="ng-binding">'.$total_subtotal.'</b></td><td id="subTotalTd"><b class="ng-binding">'.$total_discount.'</b></td>'.$extratd.'<td><b class="ng-binding">'.$total_bill_amount.'</b></td><td><b class="ng-binding">'.$total_round.'</b></td><td><b class="ng-binding">'.$total_roundoff.'</b></td><td></td></tr></tbody></table></div>';
	    ...
	    // set default header data
	   	// $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE.' 001', PDF_HEADER_STRING, array(0,64,255), array(0,64,128));
	    $pdf->SetHeaderData('', '', 'Sales Report', '', array(0,64,255), array(0,64,128));
	    $pdf->setFooterData(array(0,64,0), array(0,64,128)); 
    ...
    public function get_daily_sales_by_branch_pdf()
	{
		...
		$reportHtml .= '<div class="ng-binding"><table id="daily_sales_table" class="table table-condensed table-hover"><thead><tr><th class="hidden-xs">Created</th><th class="hidden-xs">Bill No.</th><th class="hidden-xs">Net Amt.</th><th class="hidden-xs">Discount</th>';
		...
		foreach ($list as $cal_date) {			
			...
			$reportHtml .= '<tr ng-repeat="row in saleslist" class="ng-scope"><td class="hidden-xs ng-binding">'.$response['created'].'</td><td class="hidden-xs ng-binding">'.$response['order_code'].'</td><td class="hideen-xs ng-binding">'.$response['sub_total'].'</td><td class="hideen-xs ng-binding">'.$response['discount'].'</td>';
			...
			$this->load->model('tax_main_model');
		 	...
		 	foreach ($tax_list as $tax) {
		 		...
		 		$tax_data = $this->order_model->get_tax_data_by_date_and_tax_id($cal_date,$tax_id);
		 		 ...
		 		 $reportHtml .= '<td class="hidden-xs">'.$tax['tax_percent'].'</td>';
	...
	public function report_branch_pdf()
	{
	    ...
	    // set default header data
	   	// $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE.' 001', PDF_HEADER_STRING, array(0,64,255), array(0,64,128));
	    $pdf->SetHeaderData('', '', 'Branch Report', '', array(0,64,255), array(0,64,128));
	    $pdf->setFooterData(array(0,64,0), array(0,64,128)); 
