...
class Storeinward extends CI_Controller
{
	...
	public function create_storeinward()
	{
		...
		if(isset($_POST["store_product_inward_id_arr"]) && $_POST["store_product_inward_id_arr"]!='')
		{
			$store_product_inward_id_arr = explode(',',$_POST["store_product_inward_id_arr"]);

			// echo '<pre>';
			// print_r($store_product_inward_id_arr);die;

			$inserted_store_product_inward_id_arr = array();

			foreach ($store_product_inward_id_arr as $store_product_inward_id) {
				
				//store_product_inward_id;
				$data = array();
				$data['store_product_inward_id'] = $store_product_inward_id;	

				//print_r($_POST['purchase_qty']);
				...
				if(isset($_POST['is_available_'.$store_product_inward_id]) && $_POST['is_available_'.$store_product_inward_id]=="on")
				{
					$data['is_available'] = "Y";

					if(isset($_POST['purchase_qty_'.$store_product_inward_id]) && $_POST['purchase_qty_'.$store_product_inward_id]!="")
					{
						$data['purchase_qty'] = $_POST['purchase_qty_'.$store_product_inward_id];	
					}	
					else
					{
						$data['purchase_qty'] = 0;
						...
						if($insert_id>0)
						{
							$inserted_store_product_inward_id_arr[] = $insert_id;

							// insert into store instock
							// the purchased qty would be instock for store

							$store_instock = array();
							$store_instock['store_product_inward_id'] = $data['store_product_inward_id'];
				...
				// 	$present_store_products = $this->storeinward_model->get_inwarded_store_product_ids_by_date($data['created']);

				// 	$present_store_product_id_arr = array();

				// 	foreach ($present_store_products as $store_product_idArr) 
				// 	{						
				// 		array_push($present_store_product_id_arr, $store_product_idArr['store_product_inward_id']);
				// 	}
				...
				// 	if(in_array($store_product_inward_id,$present_store_product_id_arr))
				// 	{
				// 		// remove 
				...
				if(isset($_POST['price_'.$store_product_inward_id]) && $_POST['price_'.$store_product_inward_id]!="")
				{

					$data['price'] = $_POST['price_'.$store_product_inward_id];

					$price = array();
					$price['store_product_inward_id']=$data['store_product_inward_id'];
					$price['price']=$data['price'];


					$insert_id = $this->storeproduct_model->update_price_store_product_inward($price);
			...
			// if(array_filter($present_store_product_id_arr))
			// {
				$response['status'] = "1";
