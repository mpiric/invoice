...
class Order_model extends CI_Model
{
    ...
    {
        $query = $this->db->get_where('order_detail_live', array('order_id' => $id));
        $result = $query->row_array();
        return $result;
    }
    ...
    {
        $query = $this->db->get_where('order_items_live', array('order_id' => $id));
        $result = $query->result_array();
        return $result;
    }
...
LEFT JOIN tax_main tm ON tm.tax_id = ot.tax_id where DATE(o.order_date_time)='".$cal_date."' and ot.tax_id='".$tax_id."' ".$branch_cond."  ";
    ...
    public function get_latest_printed_bill_code()
    {
	   ...
	   if($current_month >= 4 ){
		   ...
		   $year_end = date("Y-m-d", strtotime(date("Y-m-d", strtotime($year_start)) . " + 1 year - 1 day"));
		   
	   } else {
		   
		   $temp_year_start = date('Y-04-01');
		   
		   $temp_year_end = date("Y-m-d", strtotime(date("Y-m-d", strtotime($temp_year_start)) . " + 1 year - 1 day"));
		   
		   $year_start = date("Y-m-d", strtotime(date("Y-m-d", strtotime($temp_year_start)) . " - 1 year"));
		   
		   $year_end = date("Y-m-d", strtotime(date("Y-m-d", strtotime($temp_year_end)) . " - 1 year"));
		   
	   }
	   
	 $sql = " SELECT order_code FROM order_detail WHERE is_print=1 AND branch_id = '".$branch_id."' AND DATE(created) >= '".$year_start."' AND DATE(created) <= '".$year_end."'  ORDER BY order_code DESC LIMIT 0,1";
...
CASE WHEN payment_type = 1 THEN "Cash" WHEN payment_type = 2 THEN "Credit Card" WHEN payment_type = 3 THEN "Debit Card" END AS paymentType, o.order_id,o.total_amount AS bill_amount,  
    ...
    public function check_for_previous_order($order_code)
    {
        ...
        {
            $sql = 'SELECT * FROM order_detail WHERE order_code < "'.$order_code.'" AND is_print="1" AND branch_id="'.$branch_id.'" ORDER BY order_code DESC LIMIT 1 ';
