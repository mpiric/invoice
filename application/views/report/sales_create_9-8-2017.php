						...
						<div class="row">
							...
							<div class="col-md-6">
                                ...
                                <div class="form-group">
					      		...
					      		<div class="form-group col-md-4">
					      		<input type="submit" name="Submit" class="btn btn-default" value="Generate PDF" ng-click="viewReportPDF(branch_id)"><span id="pdf_loader" style="display: none"><img src="uploads/ajax-loader.gif"></span>
					      		</div>
					      ...
					      <legend>Report</legend>
					        ...
					        <tbody>
					          <tr ng-repeat="row in saleslist">
					            ...
					            <td class="hidden-xs" align="right">{{row.order_code}}</td>
							  <td class="hidden-xs" align="right">{{row.table_no}}</td>
							  <td class="hidden-xs" align="right">{{row.number_of_person}}</td>
					            <!-- <td class="hidden-xs" >{{row.totalTax}}</td> -->
					            <td class="hideen-xs" align="right">{{row.sub_total}}</td>
					            <td class="hideen-xs" align="right">{{row.tax_free}}</td>
					            ...
					            <td class="hidden-xs" align="right">{{row.roundoff | number:2}}</td>
					          ...
					          <tr>
								...
								<td></td>
								<td align="right"><b>{{getTotalCover() }}</b></td>
								<td align="right"><b>{{ getSubtotal().toFixed(2)}}</b></td>
					          	<td align="right"><b>{{ getTaxfreetotal().toFixed(2)}}</b></td>
					          	<td></td>
					          	<td></td>

					          	<!-- id="subTotalTd" --><td  align="right"><b>{{ gettotalDiscountTotal().toFixed(2)}}</b></td>
					          	...
					          	<td align="right"><b>{{ getbill_amountTotal().toFixed(2) }}</b></td>
					          	
					          	<td align="right"><b>{{ getRound().toFixed(2) }}</b></td>


					          	<td align="right"><b>{{ getroundoffTotal().toFixed(2) }}</b></td>
					          	<td></td>
					          </tr>
					        </tbody>
					        </table>
