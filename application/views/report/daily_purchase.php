...
<div class="container-fluid container-fullw" ng-controller="dailyPurchaseReportCtrl">
	<div class="row">
		<div class="col-md-12" >
				            ...
				            <div class="input-group">
				              <input type="text" name="filterdate" class="form-control" datepicker-popup="dd/MM/yyyy" ng-model="start" is-open="startOpened" ng-init="startOpened = false" min-date="'1970-12-31'" max-date="end"  close-text="Close"  ng-click="startOpen($event)" datepicker-options="dateOptions" value="<?php echo date('Y-m-d'); ?>" />
								...
								<br>
					      		...
					      		<input type="submit" name="Submit" class="btn btn-primary" value="Generate" ng-click="dailyPurchaseReport()">
					      		</div>
					...
					<div class="col-md-12">
					    <fieldset>
					      ...
					      <legend>Daily Purchase</legend>
					        <div  ng-show="!is_msg">
					        <table id="daily_purchase_report" class="table table-condensed table-hover" >
						        <thead>
						          <tr>
							          <th class="hidden-xs" align="center">Product Name</th>
							          <th class="hidden-xs" align="center">Unit</th>
							          <th class="hidden-xs" align="center">Instock</th>
							          <th class="hidden-xs" align="center">Price</th>
							          <th class="hidden-xs" align="center">Purchased Today</th>
						          </tr>
						        </thead>
						        <tbody>
						        
						            <tr ng-repeat="row in daily_purchase">
						            	<td class="hidden-xs" >{{row.name}}</td>
				            			<td class="hidden-xs" >{{row.unit}}</td>
				            			<td class="hidden-xs" >{{row.instock}}</td>
				            			<td class="hidden-xs" >{{row.price}}</td>
				            			<td class="hidden-xs" >{{row.today_qty}}</td>
						            </tr>
